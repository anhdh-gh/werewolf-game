...
**Files:**
- Create: `package.json`, `src/app/layout.tsx`, `src/app/page.tsx`, `src/app/globals.css`,
  `postcss.config.mjs`, `.env.example`, `components.json`, `src/lib/utils.ts`,
...
```bash
git add package.json package-lock.json src/app postcss.config.mjs .env.example tsconfig.json next-env.d.ts components.json src/lib/utils.ts src/components/ui
